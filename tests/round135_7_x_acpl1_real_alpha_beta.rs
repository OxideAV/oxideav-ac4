//! Round 135 — 7_X (7.0 immersive) SIMPLE/ASPX_ACPL_1 multichannel
//! encoder with **real per-parameter-band α + β extraction**.
//!
//! The 7_X immersive counterpart of the round-132 5_X real-α+β path
//! ([`Ac4ImsEncoder::encode_frame_pcm_5_0_acpl1_real_alpha_beta`]). The
//! round-118 7_X ASPX_ACPL_1 encoder emitted both `acpl_data_1ch()`
//! parameter sets at the zero-delta scaffold; round 135 upgrades them to
//! carry the analytic α (from the L/Ls and R/Rs MDCT-energy correlation,
//! §5.7.7.5 Pseudocode 116) plus the β magnitude that closes the
//! surround/carrier energy balance (§5.7.7.6.1 Pseudocode 117):
//!
//! ```text
//!   E[Ls²] = 0.5 · E[L²] · ( (1 − α)² + β² )
...
//! These tests confirm:
//!
//!   1. The new entry point produces a 7-channel AudioFrame round-trip.
//!   2. The decoder resolves the frame to `SevenXCodecMode::AspxAcpl1`
//!      with `b_has_lfe = false` and persists both `acpl_data_1ch_pair`
//!      slots plus the joint-MDCT residual layer.
//!   3. When the surround energy exceeds what α alone can model (loud
//!      anti-phase surround), the body still round-trips cleanly.
...
#[test]
fn encode_7_0_acpl1_real_alpha_beta_produces_7_channel_audio_frame() {
    ...
    assert!(
        ...
        "real-α+β 7_X encoder must produce non-empty output"
    );
...
#[test]
fn encode_7_0_acpl1_real_alpha_beta_decoder_resolves_full_body() {
    let params = CodecParameters::audio(CodecId::new("ac4"));
    let mut dec = Ac4Decoder::new(&params);
    let mut enc = Ac4ImsEncoder::new();
    let l = make_tone(220.0, 0.3);
...
/// regime (E[Ls²] > 0.5·E[L²]·(1−α)²). The frame must still round-trip to
/// a 7-channel AudioFrame and resolve as AspxAcpl1 — confirming the extra
/// β layer doesn't break the 7_X substream-body layout or the IMDCT path.
#[test]
fn encode_7_0_acpl1_real_alpha_beta_loud_antiphase_surround_round_trips() {
    let params = CodecParameters::audio(CodecId::new("ac4"));
    let mut dec = Ac4Decoder::new(&params);
    let mut enc = Ac4ImsEncoder::new();
...
/// Silence input round-trips through the real-α+β 7_X path — both α and
/// β default to the zero-codebook index in every band.
#[test]
fn encode_7_0_acpl1_real_alpha_beta_silence_round_trips() {
    let params = CodecParameters::audio(CodecId::new("ac4"));
    let mut dec = Ac4Decoder::new(&params);
    let mut enc = Ac4ImsEncoder::new();
    let z = vec![0.0f32; N];
    let frame_bytes = enc.encode_frame_pcm_7_0_acpl1_real_alpha_beta(&[&z, &z, &z, &z, &z, &z, &z]);
    let pkt = Packet::new(0, TimeBase::new(1, 48_000), frame_bytes);
    dec.send_packet(&pkt).expect("send_packet");
    let Frame::Audio(af) = dec.receive_frame().expect("receive_frame") else {
        panic!("expected audio frame");
...
#[test]
fn encode_7_0_acpl1_real_alpha_beta_is_deterministic() {
    let l = make_tone(220.0, 0.3);
    let r = make_tone(440.0, 0.3);
    let c = make_tone(660.0, 0.3);
    let ls = make_tone(880.0, 0.3);
    ...
    let f1 = enc1.encode_frame_pcm_7_0_acpl1_real_alpha_beta(&[&l, &r, &c, &ls, &rs, &lb, &rb]);
    let f2 = enc2.encode_frame_pcm_7_0_acpl1_real_alpha_beta(&[&l, &r, &c, &ls, &rs, &lb, &rb]);
    assert_eq!(
        f1, f2,
